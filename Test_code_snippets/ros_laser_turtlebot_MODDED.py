...
Beta = 1
Robot_radius = 0.21 #m
d_safe = 0.4 #in meters circle radius plus some extra offset
eps = 0.1        #in meters
...
def angular_vel(goal, constant=2):
...
def angular_vel_modded(goal, constant=4):
    a=steering_angle(goal)
...
def H_inv(Controller, yaw_angle):
    ...
    new_Controller = np.matmul(B, u)#.flatten()
...
#        print()
        ...
        if (Condition_7 == True):    #Reached
            ...
            if (Condition_6 == True):
                ...
                print("UWF_Clk")
                vel_msg.linear.x = Ucwf[0]
                vel_msg.linear.y = Ucwf[1]
                vel_msg.angular.z = 0
            ...
            if (Condition_6 == True):
                ...
                vel_msg.linear.x = Uccwf[0]
                vel_msg.linear.y = Uccwf[1]
                vel_msg.angular.z = 0
            ...
            if (Condition_6 == True):
                print("GTG-AVO")
                Stored_robot_state = robot_state
                #current_input[0] = -Uavo[0]
                #current_input[1] = -Uavo[1]
                #current_input[2] = 0
