...
def Ugtg_controller(desired_state, robot_state):
    v0 = 0.17  #m/s  max vel is given as (vx**2 + vy**2)**0.5 = 0.5 assuming vx=vy and solving it we get 0.5/2**0.5 = 0.35
    Beta = 1
    ...
    Kg = min(max(v0*(1 - np.exp(-Beta*np.absolute(error_gtg)))/np.absolute(error_gtg), 0), 0.25)
...
def Uavo_controller(filtered_obst_pts, sensors_dist, robot_state): #yaw_angle
    ...
    for i in range(len(filtered_obst_pts)):
        ...
        Ko = min(max((1/np.absolute(error_avo))*(C/((np.absolute(error_avo)**2)+eps)), 0), 0.05)
...
def move2coord(X_coord,Y_coord, Dbug):
    ...
    desired_state[0] = X_coord #float(input("Set your x goal: ")) # Get the input from the user.
    desired_state[1] = Y_coord #float(input("Set your y goal: "))
...
#        print()
        ...
        #dif_degree = np.rad2deg(steering_angle([X_coord,Y_coord]))
        dif_degree = 0
        ...
        Left_coord = filtered_obst_pts[89]
        Right_coord = filtered_obst_pts[269]
        Forward_coord = filtered_obst_pts[0]
        Back_coord = filtered_obst_pts[179]
        
        Left_range = sensor_Dist[89]
        Right_range = sensor_Dist[269]
        Forward_range = sensor_Dist[0]
        Back_range = sensor_Dist[179]
        dis = np.where(sensor_Dist < d_safe-eps)
        
        #Left_half = sensor_Dist[20:140]
        Q1 = sensor_Dist[0:90]
        Q2 = sensor_Dist[91:180]
        Q3 = sensor_Dist[181:270]
        Q4 = sensor_Dist[271:360]
        
        
        
        Front_half = np.mean(np.concatenate((Q1,Q4)))
        back_half = np.mean(np.concatenate((Q2,Q3)))
        
        Left_half = np.mean(np.concatenate((Q1,Q2)))
        Right_half = np.mean(np.concatenate((Q3,Q4)))
        
        Condition_6 = any((element < d_safe-eps)  for element in sensor_Dist) and (Front_half <=0.75 or back_half <=0.75 or Left_half <=0.75 or Right_half <=0.75)    #sensor dist assumed as x-xo
        
        #dis = np.where(Left_half < 1.0)
        #print()
        #print(len(dis[0]))
        #print("F:", Front_half, "B:", back_half, "L:", Left_half, "R:", Right_half)
        #print()
        ...
        if (Condition_7 == True):    #Reached
        ...
        elif (Condition_1 == True and (Condition_2 < 90) == True and (np.round(Front_half,1) != np.round(Left_half,1) and np.round(Front_half,1) != np.round(Right_half,1) and (np.round(Front_half,1) != np.round(back_half,1)))):  #switch to Ucwf
        #elif (Condition_1 == True and (Condition_2 < 90) == True and ((Front_half <= 0.75 ) or (Left_half <= 0.75 )) ):  #switch to Ucwf
            ...
            if (Condition_6 == True):
                ...
                print("UWF")
                #if (Left_half <= d_safe+eps):# and (Right_half > 0.75):
                #Ucwf_apply = H_inv(Ucwf, yaw_angle)
                
                if (Right_half < Left_half): #and abs(dif_degree)-abs(current_degree) < 0:
                    #print("Right_CC")
                    Ucwf_apply = H_inv(Uccwf, yaw_angle)  
                
                elif (np.round(Right_half,1) == np.round(Left_half,1)) and (Right_half < 1.0):
                    #print("FW_CC")
                    Ucwf_apply = H_inv(Ugtg, yaw_angle)
             
                elif (Left_half < Right_half) and (Left_half < 7.5) and abs(dif_degree)-abs(current_degree) < 0:
                    #print("Left_CC")
                    Ucwf_apply = H_inv(Uccwf, yaw_angle)   
                else:
                    #print("Left_C")
        ...
        elif (Condition_1 == True and (Condition_3 > 90) == True and  (np.round(Front_half,1) != np.round(Left_half,1) and np.round(Front_half,1) != np.round(Right_half,1) and (np.round(Front_half,1) != np.round(back_half,1))) ):  #switch to Uccwf
        ...
        if Dbug==1:
            ...
            #print("Angle Error:", dif_degree)
            #print("Current Angle (Deg): ",current_degree)   
            #print("Angle Diff (Deg): ",abs(dif_degree)-abs(current_degree))          
...
def Auto_SLAM():
    ...
    X_coordinates = [3, 3,   5,   3,   3, 3,    4.8,  4.8, 4.8,  3,    3,   0,  3,  3,    4.2,   4.2, 5,  4.2, 4.2,  1,    1,    0.5,    3,    3,  1,  3,  3,    2,      2,     3, 3, 0]
    Y_coordinates = [0, 2.5, 3,   2.5, 0, -0.5, -0.5, 1,   -0.5, -0.4, -2,  -2, -2, -3.5, -3.5,  -5,  -5, -5,  -3.5, -3.7, -5.2, -5.2, -5.5, -7, -7, -7, -5.5, -5.5, -4,   -4, 0, 0]


    for i in range (0,len(X_coordinates)):                                    #i from 0 to 3
        move2coord(X_coordinates[i],Y_coordinates[i],1)
